...
const configContent = fs.readFileSync(
  path.join(__dirname, "../voiceover-customers-config.ts"),
  "utf8"
);
const cleaned = configContent
  .replace(/export const CUSTOMERS_SCENES =/, "var CUSTOMERS_SCENES =")
  .replace(/ as const;/, ";")
  .replace(/^\/\/.*/gm, "");
eval(cleaned);
...
async function main() {
  ...
  for (let i = 0; i < CUSTOMERS_SCENES.length; i++) {
    await generateOne(CUSTOMERS_SCENES[i]);
    if (i < CUSTOMERS_SCENES.length - 1) {
      const nextPath = path.join(OUT_DIR, `${CUSTOMERS_SCENES[i + 1].id}.wav`);
