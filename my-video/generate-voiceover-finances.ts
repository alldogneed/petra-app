/**
 * Generates Hebrew voiceover for the Finances module tutorial using Google Gemini 2.5 TTS.
 *
 * Usage:
 *   GEMINI_KEY=AIza... npx tsx generate-voiceover-finances.ts
 *
 * Output: public/voiceover/{scene-id}.wav
 */

import { writeFileSync, mkdirSync, existsSync } from "fs";
import { FINANCES_SCENES } from "./voiceover-finances-config.js";
...
// MUST be identical to all other Petra videos — keeps voice character consistent
const HEBREW_PREFIX =
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.error("    Run: GEMINI_KEY=AIza... npx tsx generate-voiceover-finances.ts");
    process.exit(1);
  }

  mkdirSync(OUTPUT_DIR, { recursive: true });
  console.log(`\n₪ Petra Finances Tutorial Voiceover — Gemini 2.5 TTS (${VOICE})\n`);

  for (let i = 0; i < FINANCES_SCENES.length; i++) {
    const scene = FINANCES_SCENES[i];
    ...
    if (i < FINANCES_SCENES.length - 1) {
      const next = FINANCES_SCENES[i + 1];
  ...
  console.log(`\n✅ Done! Refresh Remotion Studio to see the Finances tutorial.`);
}
