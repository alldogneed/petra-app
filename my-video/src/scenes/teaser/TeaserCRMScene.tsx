...
export const TeaserCRMScene: React.FC = () => {
  ...
  // Zoom toward lead cards (right side in RTL = first column)
  const zoomP = spring({ frame: frame - 54, fps, config: { damping: 320, stiffness: 45 } });
  const zoomScale = interpolate(zoomP, [0, 1], [1.0, 1.16]);
  ...
  return (
    <AbsoluteFill style={{ background: "#f8fafc", fontFamily: FONT, direction: "rtl" }}>
              ...
              return (
                <div key={col.label} style={{ flex: 1, opacity: colOpacity, transform: `translateY(${colY}px)`, display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  {col.cards.map((card, ci) => {
                    ...
                    // Glow on first card of first column
                    const isHighlighted = col.delay === 55 && ci === 0;
                    const glowP = isHighlighted ? interpolate(frame, [cardDelay + 8, cardDelay + 32], [0, 1], { extrapolateRight: "clamp" }) : 0;
                    const glowOpacity = isHighlighted ? interpolate(glowP, [0, 0.4, 1], [0, 1, 0]) : 0;
                    ...
                    return (
                      <div key={card.name} style={{
                        ...
                        boxShadow: glowOpacity > 0.05 ? `0 0 14px rgba(234,88,12,${glowOpacity * 0.5})` : "none",
                      }}>
      ...
      {/* Cursor clicks on first lead card */}
      <CursorAnimation
