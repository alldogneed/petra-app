/**
 * Generates Hebrew voiceover for the Orders module tutorial using Google Gemini 2.5 TTS.
 *
 * Usage:
 *   GEMINI_KEY=AIza... npx tsx generate-voiceover-orders.ts
 *
 * Output: public/voiceover/{scene-id}.wav
 */

import { writeFileSync, mkdirSync, existsSync } from "fs";
import { ORDERS_SCENES } from "./voiceover-orders-config.js";
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.error("    Run: GEMINI_KEY=AIza... npx tsx generate-voiceover-orders.ts");
    process.exit(1);
  }

  mkdirSync(OUTPUT_DIR, { recursive: true });
  console.log(`\n📦 Petra Orders Tutorial Voiceover — Gemini 2.5 TTS (${VOICE})\n`);

  for (let i = 0; i < ORDERS_SCENES.length; i++) {
    const scene = ORDERS_SCENES[i];
    ...
    if (i < ORDERS_SCENES.length - 1) {
      const next = ORDERS_SCENES[i + 1];
  ...
  console.log(`\n✅ Done! Refresh Remotion Studio to see the Orders tutorial.`);
}
