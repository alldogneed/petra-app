/**
 * Generates Hebrew voiceover for the Sales/Leads tutorial using Google Gemini 2.5 TTS.
 *
 * Usage:
 *   GEMINI_KEY=AIza... npx tsx generate-voiceover-sales.ts
 *
 * Output: public/voiceover/{scene-id}.wav
 */

import { writeFileSync, mkdirSync, existsSync } from "fs";
import { SALES_SCENES } from "./voiceover-sales-config.js";
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.error("    Run: GEMINI_KEY=AIza... npx tsx generate-voiceover-sales.ts");
    process.exit(1);
  }

  mkdirSync(OUTPUT_DIR, { recursive: true });
  console.log(`\n🐾 Petra Sales Tutorial Voiceover — Gemini 2.5 TTS (${VOICE})\n`);

  for (let i = 0; i < SALES_SCENES.length; i++) {
    const scene = SALES_SCENES[i];
    ...
    if (i < SALES_SCENES.length - 1) {
      const next = SALES_SCENES[i + 1];
  ...
  console.log(`\n✅ Done! Refresh Remotion Studio to see the Sales tutorial.`);
}
