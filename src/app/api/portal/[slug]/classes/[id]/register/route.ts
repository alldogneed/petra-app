...
// POST /api/portal/[slug]/classes/[id]/register — requires ACTIVE membership
export async function POST(
  ...
  try {
    ...
    const { status } = await registerForClass(
      business.id,
      membership!.id,
      params.id
    );
    ...
    return portalErrorResponse(error, "portal class register POST");
  }
}
