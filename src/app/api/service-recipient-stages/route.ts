...
const DEFAULT_STAGES = [
  ...
  { key: "APPROVED",       name: "מאושר",           color: "bg-amber-100 text-amber-700",    sortOrder: 2, isBuiltIn: true },
  { key: "WAITLIST",       name: "רשימת המתנה",     color: "bg-slate-100 text-slate-600",    sortOrder: 3, isBuiltIn: true },
  { key: "MATCHED",        name: "שובץ",            color: "bg-blue-100 text-blue-700",      sortOrder: 4, isBuiltIn: true },
  { key: "JOINT_TRAINING", name: "אימון משותף",     color: "bg-indigo-100 text-indigo-700",  sortOrder: 5, isBuiltIn: true },
  { key: "ACTIVE",         name: "פעיל",            color: "bg-emerald-100 text-emerald-700", sortOrder: 6, isBuiltIn: true },
  { key: "CLOSED",         name: "סגור",            color: "bg-red-100 text-red-600",        sortOrder: 7, isBuiltIn: true },
];

async function ensureStages(businessId: string) {
  const existing = await prisma.serviceRecipientStage.count({ where: { businessId } });
  if (existing === 0) {
    await prisma.serviceRecipientStage.createMany({
      data: DEFAULT_STAGES.map((s) => ({ ...s, businessId })),
    });
  }
}
