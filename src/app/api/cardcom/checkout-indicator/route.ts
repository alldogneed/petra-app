...
// Only basic and pro may be activated via this immediate-payment webhook
...
/**
 * GET /api/cardcom/checkout-indicator
 *
 * Cardcom calls this after a successful immediate payment (Operation=1).
 * Handles two flows depending on the UserId:
 *
 *   A) "pending:{checkoutId}::{tier}" — checkout-first (new user):
 *      → Creates PlatformUser + Business, activates subscription immediately,
 *        sends welcome email with temp password
 *
 *   B) "{businessId}::{tier}" — existing logged-in user:
 *      → Saves token + activates subscription immediately (30-day period)
 *
...
export async function GET(request: NextRequest) {
  ...
  const sigValid = providedSig ? verifyIndicatorSignature("/api/cardcom/checkout-indicator", providedSig) : false;
  ...
  if (!sigValid && !legacyValid) {
    console.error(`checkout-indicator [L1]: invalid secret from IP ${ip}`);
    await prisma.subscriptionEvent.create({
      data: { businessId: "unknown", eventType: "security_invalid_secret", ipAddress: ip, metadata: { path: "checkout-indicator" } },
    }).catch(() => null);
  ...
  if (!checkRateLimit(ip, 10)) {
    console.warn(`checkout-indicator [L2]: rate limit exceeded for IP ${ip}`);
    await prisma.subscriptionEvent.create({
  ...
  const existing = await prisma.subscriptionEvent.findFirst({
    where: { lowprofileCode: lowProfileCode, eventType: "checkout_activate" },
    select: { id: true },
  });
  if (existing) {
    console.log(`checkout-indicator [L3]: duplicate code ${lowProfileCode} — skipping`);
  ...
  if (data.DealResponse !== "0") {
    console.warn(`checkout-indicator [L4]: DealResponse=${data.DealResponse} for ${lowProfileCode}`);
    await prisma.subscriptionEvent.create({
  ...
  if (rawUserId.startsWith("pending:")) {
    ...
    if (colonIdx === -1) {
      console.error(`checkout-indicator [FlowA]: malformed UserId: ${rawUserId}`);
      return new Response("OK");
    }
    ...
    if (!isValidTier(tier) || !BILLABLE_TIERS.has(tier)) {
      console.error(`checkout-indicator [FlowA]: invalid/non-billable tier ${tier}`);
      return new Response("OK");
    }
    ...
    if (!checkout) {
      console.error(`checkout-indicator [FlowA]: PendingCheckout ${checkoutId} not found`);
      return new Response("OK");
    }
    if (checkout.processed) {
      console.log(`checkout-indicator [FlowA]: already processed ${checkoutId}`);
      return new Response("OK");
    }
    if (checkout.expiresAt < now) {
      console.warn(`checkout-indicator [FlowA]: PendingCheckout ${checkoutId} expired`);
      return new Response("OK");
    }
    ...
    if (existingUser) {
      console.warn(`checkout-indicator [FlowA]: email ${checkout.email} already registered, skipping`);
    ...
    // ── Activate subscription immediately (30-day period) ─────────────────
    ...
    await logAudit({
      ...
      targetId:     user.id,
      metadata:     { email: user.email, name: user.name, method: "checkout_first_immediate" },
    });
    ...
    await sendCheckoutWelcomeEmail({
      ...
      tempPassword,
    }).catch((e) => console.error("checkout-indicator: failed to send welcome email:", e));

    console.log(`checkout-indicator [FlowA]: created user ${user.id} for ${checkout.email}, activated ${tier} subscription`);
    return new Response("OK");
  }
  ...
  if (!businessId || !isValidTier(tier) || !BILLABLE_TIERS.has(tier)) {
    console.error(`checkout-indicator [FlowB]: invalid/non-billable UserId: ${rawUserId}`);
    return new Response("OK");
  }

  const business = await prisma.business.findUnique({ where: { id: businessId }, select: { id: true } });
  if (!business) {
    console.error(`checkout-indicator [FlowB]: business not found: ${businessId}`);
    return new Response("OK");
  }
